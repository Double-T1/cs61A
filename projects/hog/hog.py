...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score[0] < goal and score[1] < goal:
        opponent = other(who)
        number_rolls = strategy[who](score[who], score[opponent])
        if feral_hogs and abs(number_rolls - prev_score[who]) == 2:
            score[who] += 3
        prev_score[who] = take_turn(number_rolls, score[opponent], dice)
        score[who] += prev_score[who]
        if is_swap(score[who], score[opponent]):
            score[who], score[opponent] = score[opponent], score[who]
        who = opponent
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        return announce_highest(who, score[who], max(running_high, diff))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num_rolls in range(1, 11):
        cur_average = averaged_dice(num_rolls, dice)
        if cur_average > max_average:
            max_average = cur_average
            max_average_roll = num_rolls
    return max_average_roll
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    addition_when_roll_0 = free_bacon(opponent_score)
    if is_swap(addition_when_roll_0 + score, opponent_score):
        if addition_when_roll_0 + score > opponent_score:
            return num_rolls
        if addition_when_roll_0 + score < opponent_score:
            return 0
...
def final_strategy(score, opponent_score, cutoff=5, num_rolls=6):
    ...
    *** YOUR DESCRIPTION HERE ***
    The function returns the number of rolls that statistically achieves the 
    highest win rate. Provding that the baseline is always_roll(4).
