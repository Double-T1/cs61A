...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(cur_coin, total):
        if total < cur_coin:
            return 0
        if total == cur_coin:
            return 1
        return helper(cur_coin*2, total) + helper(cur_coin, total-cur_coin)
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    "empty stick!! => not start or end stick"
    if n == 1:
...
def make_anonymous_factorial():
    ...
    return lambda val: \
        (lambda func, val: 
            func(func, val)
        )(lambda fib, val: 1 if val == 1 else val * fib(fib, val-1), val)
