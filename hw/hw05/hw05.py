...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    asset = make_bank(balance)
    attempts = []
    def account(amount, password_attempt):
        nonlocal attempts
        if len(attempts) == 3:
            return 'Too many incorrect attempts. Attempts: [\'{p1}\', \'{p2}\', \'{p3}\']'\
                .format(p1=attempts[0], p2=attempts[1], p3=attempts[2])
        if password_attempt != password:
            attempts.append(password_attempt)
            return 'Incorrect password'
        return asset('withdraw',amount)
    return account
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    def joint_withdraw(amount, password_attempt):
        if password_attempt == old_pass or password_attempt == new_pass:
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    for modulo in range(m):
        def m_generators():
            natural_num = naturals()
            while True:
                yield m*next(natural_num) - (m-modulo)%m 
        yield m_generators()
