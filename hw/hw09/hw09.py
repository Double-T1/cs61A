...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    term = [odd_term, even_term]
    def helper(cur, cur_term):
        if cur == n:
            return term[cur_term](cur)
        return term[cur_term](cur) + helper(cur+1,cur_term^1)
    return helper(1, 0)
...
def mutate_reverse(link):
    ...
    def get_firsts(link):
        if not link:
            return 
        get_firsts(link.rest)
        firsts.append(link.first)
        return
    get_firsts(link)
