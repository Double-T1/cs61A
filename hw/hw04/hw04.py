...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(size(m))
    l, r = totals_tree(end(left(m))), totals_tree(end(right(m)))
    return tree(label(l)+label(r),[l,r])
...
def replace_leaf(t, find_value, replace_value):
    ...
    return tree(label(t),[replace_leaf(child, find_value, replace_value) for child in branches(t)])
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y)*upper_bound(y) > 0, 'can\'t be divided by a 0'
...
def quadratic(x, a, b, c):
    ...
    l_map, u_map = f(l), f(u) 
    new_l, new_u = min(l_map,u_map), max(l_map,u_map)
    y = -b / (2*a)
    if y > l and y < u:
        new_l = min(new_l,f(y))
        new_u = max(new_u,f(y))
    return interval(new_l,new_u)
...
def check_par():
    ...
    r2 = interval(13, 20) # Replace this line!
    return r1, r2
