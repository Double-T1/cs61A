...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        # Insert
        insert_edits, insert_cost = helper_align(skeleton_idx, code_idx+1) 
        insert_edits = "+[" + code_char + "]" + insert_edits
        possibilities.append((insert_edits, insert_cost + 1))
        # Delete
        delete_edits, delete_cost = helper_align(skeleton_idx+1, code_idx)
        delete_edits = "-[" + skel_char + "]" + delete_edits
        possibilities.append((delete_edits, delete_cost + 1))
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    total = sum(s)
    def helper(cur_accu, i):
        if i == len(s):
            return 1 if abs(total - cur_accu*2) <= d else 0
        return helper(cur_accu, i+1) + helper(cur_accu+s[i], i+1)
    return helper(0, 0)//2
