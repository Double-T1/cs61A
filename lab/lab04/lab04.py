...
def reverse_recursive(lst):
    ...
    def recursive(lst, index, ans):
        if index == len(lst):
            return
        recursive(lst, index+1, ans)
...
def add_chars(w1, w2):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(w1, w2, i1, i2):
        if i1 == len(w1): 
            return w2[i2:]
        if i2 == len(w2):
            return ''
        if w1[i1] == w2[i2]:
            return helper(w1, w2, i1+1, i2+1)
        return w2[i2] + helper(w1, w2, i1, i2+1)
